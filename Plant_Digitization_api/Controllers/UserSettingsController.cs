    ...
    public class UserSettingsController : ApiController
    {
        ...
        public IHttpActionResult Function_details(Models.Function S)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionstring))
            {
                ...
                {
                    database_connection d = new database_connection();
                    string con_string = d.Getconnectionstring(S.CompanyCode, S.ParentPlant, S.FunctionName);
                    if (con_string == "0")
                    {
                        return ResponseMessage(Request.CreateErrorResponse(HttpStatusCode.InternalServerError, "Couldnot connect to database"));
        ...
        public IHttpActionResult Assets_details(Models.Assets asts)
        {
            ...
            else
            {
                using (SqlConnection con = new SqlConnection(con_string))
                {
                    ...
                    //cmd.Parameters.Add("@Bottle_neck", SqlDbType.NVarChar, 150).Value = asts.Bottle_neck;
                    SqlParameter SQLReturn = new SqlParameter("@SQLReturn", SqlDbType.NVarChar, 50);
                    SQLReturn.Direction = ParameterDirection.Output;
                    cmd.Parameters.Add(SQLReturn);
                    cmd.ExecuteNonQuery();
        ...
        public IHttpActionResult Product_details(Models.Products p)
        {
            ...
            try
            {
                ...
                if (con_string == "0")
                {
                    return ResponseMessage(Request.CreateErrorResponse(HttpStatusCode.InternalServerError, "Could not connect to database"));
                }

                using (SqlConnection con = new SqlConnection(con_string))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_Masterproduct", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandTimeout = 0;

                        // Add parameters safely, handling possible nulls
                        cmd.Parameters.AddWithValue("@QueryType", p.QueryType ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@M_ID", p.M_ID ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@Variant_Code", p.Variant_Code ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@VariantName", p.VariantName ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@VariantDescription", p.VariantDescription ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@OperationName", p.OperationName ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@Machine_Code", p.Machine_Code ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@RecipeName", p.RecipeName ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@CycleTime", p.CycleTime ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@CompanyCode", p.CompanyCode ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@PlantCode", p.PlantCode ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@LineCode", p.Line_Code ?? (object)DBNull.Value);
                        cmd.Parameters.Add("@Cost", SqlDbType.Decimal).Value = p.Cost;
                        cmd.Parameters.AddWithValue("@autocycletime", p.Autocycletime ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@manualcycletime", p.Manualcycletime ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@Ideal_cycletime", p.Idlecycletime ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@No_of_fixtures", p.Nooffixtures ?? (object)DBNull.Value);

                        // Handle image data safely
                        string imageFormat = "Unknown";
                        if (p.ImgData != null && p.ImgData.Length > 0)
                        {
                            try
                            {
                                using (MemoryStream ms = new MemoryStream(p.ImgData))
                                {
                                ...
                                cmd.Parameters.AddWithValue("@Imgdata", p.ImgData);
                            }
                            catch (Exception)
                            {
                                imageFormat = "Invalid Format";
                                cmd.Parameters.Add("@Imgdata", SqlDbType.VarBinary).Value = DBNull.Value;
                            }
                        }
                        else
                        {
                        ...
                        cmd.Parameters.AddWithValue("@ImageFormat", imageFormat);

                        // Output parameter for SQL response
                        SqlParameter sqlReturn = new SqlParameter("@SQLReturn", SqlDbType.NVarChar, 50)
                        {
                        ...
                        string response = sqlReturn.Value.ToString().Trim();
                        //return Ok(new { Response = response, ImageFormat = imageFormat });
                        return Ok(response);
                    }
                }
            }
            ...
            {
                return ResponseMessage(Request.CreateErrorResponse(HttpStatusCode.InternalServerError, "Database error: " + ex.Message));
            }
            catch (Exception ex)
            {
        ...
        //            cmd.Parameters.Add("@Imgdata", SqlDbType.VarBinary).Value = p.ImgData;
        //            SqlParameter SQLReturn = new SqlParameter("@SQLReturn", SqlDbType.NVarChar, 50);
        //            SQLReturn.Direction = ParameterDirection.Output;
        //            cmd.Parameters.Add(SQLReturn);
        //            cmd.ExecuteNonQuery();
        ...
        public IHttpActionResult Users_details(Models.User u)
        {
            using (SqlConnection con = new SqlConnection(connectionstring))
            {
                ...
                cmd.Parameters.Add("@UserName", SqlDbType.NVarChar, 150).Value = u.UserID; //added for IOT STD
                SqlParameter SQLReturn = new SqlParameter("@SQLReturn", SqlDbType.NVarChar, 50);
                SQLReturn.Direction = ParameterDirection.Output;
                cmd.Parameters.Add(SQLReturn);
                cmd.ExecuteNonQuery();
...
                }
        ...
        [HttpPost]
        public IHttpActionResult Target_Production(Models.Production P)
        {
            database_connection d = new database_connection();
            string con_string = d.Getconnectionstring(P.CompanyCode, P.PlantCode, P.Line_Code);
            if (con_string == "0")
            ...
            else
            {
                using (SqlConnection con = new SqlConnection(con_string))
                {
                    string response = string.Empty;
                    con.Open();
                    SqlCommand cmd = new SqlCommand("SP_Target_Production", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandTimeout = 0;
                    cmd.Parameters.Add("@QueryType", SqlDbType.NVarChar, 150).Value = P.QueryType;
                    cmd.Parameters.Add("@Year", SqlDbType.NVarChar, 150).Value = P.Year;
                    cmd.Parameters.Add("@Month", SqlDbType.NVarChar, 150).Value = P.Month;
                    cmd.Parameters.Add("@Shift", SqlDbType.NVarChar, 150).Value = P.Shift;
                    cmd.Parameters.Add("@Hour1", SqlDbType.NVarChar, 150).Value = P.FirstHr;
                    cmd.Parameters.Add("@Hour2", SqlDbType.NVarChar, 150).Value = P.SecondHr;
                    cmd.Parameters.Add("@Hour3", SqlDbType.NVarChar, 150).Value = P.ThirdHr;
                    cmd.Parameters.Add("@Hour4", SqlDbType.NVarChar, 150).Value = P.FourthHr;
                    cmd.Parameters.Add("@Hour5", SqlDbType.NVarChar, 150).Value = P.FifthHr;
                    cmd.Parameters.Add("@Hour6", SqlDbType.NVarChar, 150).Value = P.SixthHr;
                    cmd.Parameters.Add("@Hour7", SqlDbType.NVarChar, 150).Value = P.SeventhHr;
                    cmd.Parameters.Add("@Hour8", SqlDbType.NVarChar, 150).Value = P.EightthHr;
        ...
        [HttpPost]
        [CustomAuthenticationFilter]
        public HttpResponseMessage Target_Production_View(Models.Production P)
        {
            database_connection d = new database_connection();
            string con_string = d.Getconnectionstring(P.CompanyCode, P.PlantCode, P.Line_Code);
            if (con_string == "0")
